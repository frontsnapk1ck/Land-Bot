...
public class Commands extends PlayerCommand {
    ...
    private void say(GuildMessageReceivedEvent e) {
        ...
        else if (DisPermUtil.checkPermission(e.getMember(), DisPerm.ADMINISTRATOR)) 
        {
    ...
    private void cooldown(GuildMessageReceivedEvent e, int cooldown) 
    {
        ...
        boolean valid = DisPermUtil.checkPermission(e.getMember(), DisPerm.ADMINISTRATOR) && s.getAdminCooldownBypass();
        if (valid)
            return;
    ...
    {
        String out = "";
        String[] message = FileReader.read(AlloyUtil.ALLOY_PATH + "res\\globals\\help\\help.msg");
